...
function SortableRow({ item }: { item: SeedItem }) {
  ...
  return (
    <div
      ...
      className="flex items-center justify-between rounded-xl border border-[#2d2d2d] bg-[#2a2a2a] px-4 py-3"
    >
      <div className="flex items-center gap-3">
        <button type="button" className="cursor-grab text-[#9e9e9e]" {...attributes} {...listeners}>
          <GripVertical className="h-4 w-4" />
        </button>
        <span className="text-sm font-medium text-white">{item.label}</span>
      </div>
      <span className="text-xs text-[#9e9e9e]">{item.id}</span>
    </div>
  );
}
